...
export async function GET(request: NextRequest) {
  try {
    ...
    const templates = await prisma.competenceFileTemplate.findMany({
      where,
      orderBy: [
        { isDefault: 'desc' },
        { createdAt: 'desc' }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (validatedData.isDefault) {
      await prisma.competenceFileTemplate.updateMany({
        where: { isDefault: true },
        data: { isDefault: false }
      });
    }

    const template = await prisma.competenceFileTemplate.create({
      data: {
        ...validatedData,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const template = await prisma.competenceFileTemplate.update({
      where: { id: templateId },
      data: {
        ...validatedData,
        updatedAt: new Date(),
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const usageCount = await prisma.competenceFile.count({
      where: { templateId }
    });
    ...
    await prisma.competenceFileTemplate.delete({
      where: { id: templateId }
    });
