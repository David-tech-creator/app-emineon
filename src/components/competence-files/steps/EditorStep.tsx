...
function SegmentBlock({ segment, jobDescription, selectedCandidate }: { 
  ...
  const ContentInitializationPlugin = () => {
    ...
    React.useEffect(() => {
      // Only initialize once when the editor is first created
      ...
      console.log('🔄 ONE-TIME editor initialization for segment:', segment.id, {
      ...
      editor.update(() => {
        ...
        if (contentToCheck?.trim()) {
          try {
            ...
            const paragraph = $createParagraphNode();
            paragraph.append($createTextNode(contentToCheck || 'Enter content here...'));
    ...
    }, [editor]); // Remove segment.content dependency to prevent re-initialization
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const localizedJob = { ...(jobDescription as any) } as any;
      await regenerateSegment(segment.id, localizedJob, selectedCandidate);
    } catch (error) {
  ...
  const handleEnhance = async (action: 'improve' | 'expand' | 'rewrite') => {
    ...
    try {
      ...
      // Update the segment's content first with current editor content
      ...
      switch (action) {
        case 'improve':
          await improveSegmentWithContent(segment.id, { ...(jobDescription as any) } as any, selectedCandidate, currentContent || segment.content);
          break;
        case 'expand':
          await expandSegmentWithContent(segment.id, { ...(jobDescription as any) } as any, selectedCandidate, currentContent || segment.content);
          break;
        case 'rewrite':
          await rewriteSegmentWithContent(segment.id, { ...(jobDescription as any) } as any, selectedCandidate, currentContent || segment.content);
          break;
        default:
  ...
  const improveSegmentWithContent = async (segmentId: string, jobData: any, candidateData: any, existingContent: string) => {
  ...
  const expandSegmentWithContent = async (segmentId: string, jobData: any, candidateData: any, existingContent: string) => {
    const segment = segments.find(s => s.id === segmentId);
    if (!segment) return;
  ...
  const rewriteSegmentWithContent = async (segmentId: string, jobData: any, candidateData: any, existingContent: string) => {
    const segment = segments.find(s => s.id === segmentId);
    if (!segment) return;
...
function LivePreview({ 
  ...
  const handleSyncPreview = useCallback(() => {
    ...
    segments.forEach((segment) => {
      ...
      // Preserve rich editor state by ONLY updating htmlContent for preview
      const needsHtmlUpdate = latestHtml !== (segment as any).htmlContent;
      if (needsHtmlUpdate) {
        updateSegment(segment.id, {
    ...
    // Trigger preview refresh and reset sync indicators
  ...
  const previewHTML = useMemo(() => {
    ...
    visibleSegments.forEach(segment => {
      // Use rich HTML content from editor if available, otherwise format plain content
      const htmlContent = (segment as any).htmlContent;
      const plainContent = segment.content || '';
      
      if (!htmlContent && !plainContent) return;
      
      // Emineon primary color for all section titles
      const sectionColor = '#073C51';
      let bodyHtml = '';
      
      if (htmlContent) {
        // Use the rich HTML directly from the editor - just clean up any remaining issues
        bodyHtml = htmlContent
          .replace(/\*{2,}/g, '') // Remove any stray asterisks
          .replace(/```[\s\S]*?```/g, '') // Remove code blocks
          .replace(/`{1,3}/g, '') // Remove backticks
          .trim();
      } else {
        // Fallback to formatters for plain content
        ...
        if (isTechSection || segment.title.toUpperCase().includes('FUNCTIONAL SKILLS')) {
          if (segment.title.toUpperCase().includes('TECHNICAL EXPERTISE') || segment.title.toUpperCase().includes('FUNCTIONAL SKILLS')) {
            bodyHtml = formatTechnicalSectionPreview(plainContent);
          } else {
    ...
    const footerText = selectedTemplate === 'antaes' ? 'Powered by Antaes' : selectedTemplate === 'emineon' ? 'Powered by Emineon' : '';
    html += `
    ...
    return html;
  }, [segments, selectedCandidate, selectedTemplate, formatRegularContent, previewFont, previewFontSize]);
...
export function EditorStep({
  ...
  }, [hasInitialized, selectedCandidate, jobDescription, loadFromAI, language]);
        ...
        {fullscreenMode !== 'editor' && (
          <div 
            ...
            style={{ 
              ...
              minWidth: fullscreenMode !== 'editor' ? '300px' : '0px'
            }}
          >
            <LivePreview
              key={previewSyncKey}
